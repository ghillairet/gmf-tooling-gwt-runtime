...
import org.eclipse.gmf.runtime.gwt.services.ParserUtil;
	...
	public Command getApplyCommand(EObject source, String newValue) {
		...
		for (int i = 0; i < myFeatures.length; i++) {
			if (myFeatures[i].isMany()) {
				EList<Object> valuesList = new BasicEList<Object>();
				valuesList.addAll((Collection<?>) source.eGet(myFeatures[i]));
				result.append(RemoveCommand.create(domain, source, myFeatures[i], valuesList));
				if (newValues[i] != null) {
					result.append(AddCommand.create(domain, source, myFeatures[i], newValues[i]));
				}
			} else {
				result.append(SetCommand.create(domain, source, myFeatures[i], newValues[i] == null ? SetCommand.UNSET_VALUE : newValues[i]));
			}
		}
