...
public class ExpandDrawerCommand extends CommandWrapper { 
	...
	public ExpandDrawerCommand(View drawer) {
		DrawerStyle style = null;
		
		try {
			style = (DrawerStyle) drawer.getStyle(NotationPackage.eINSTANCE.getDrawerStyle());
		} catch (Exception e) {
		...
		if (style == null) {
			...
			myProxy = SetCommand.create(AdapterFactoryEditingDomain.getEditingDomainFor(style), style, NotationPackage.eINSTANCE.getDrawerStyle_Collapsed(), Boolean.FALSE);
		}
	}
