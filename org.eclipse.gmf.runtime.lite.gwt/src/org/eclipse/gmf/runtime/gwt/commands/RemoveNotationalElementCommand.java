...
public class RemoveNotationalElementCommand extends AbstractCommand {
	...
	protected boolean prepare() {
		...
		Iterator<Edge> it = childView.getSourceEdges().iterator(); it.hasNext(); ) {
			Edge next = it.next();
			removeEdgeCommands.append(new RemoveNotationalEdgeCommand(childView.getDiagram(), next));
		}
		for(@SuppressWarnings("unchecked")
		Iterator<Edge> it = childView.getTargetEdges().iterator(); it.hasNext(); ) {
			Edge next = it.next();
			removeEdgeCommands.append(new RemoveNotationalEdgeCommand(childView.getDiagram(), next));
		}
	...
	public void execute() {
		...
		if (removeEdgeCommands != null && !removeEdgeCommands.isEmpty()) {
