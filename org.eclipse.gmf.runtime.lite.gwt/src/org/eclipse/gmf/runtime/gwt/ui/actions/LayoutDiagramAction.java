...
import org.eclipse.gmf.runtime.gwt.services.IDiagramLayouter;
...
public class LayoutDiagramAction extends WorkbenchPartAction {
	...
	public void run() {
		...
		final Command layoutCommand = layouter.layout((GraphicalEditPart) viewer.getContents());
		if (layoutCommand != null && layoutCommand.canExecute()) {
			EditingDomain editingDomain = AdapterFactoryEditingDomain.getEditingDomainFor(viewer.getContents().getModel());
			viewer.getEditDomain().getCommandStack().execute(new AbstractWrappingCommand(editingDomain) {
				@Override
